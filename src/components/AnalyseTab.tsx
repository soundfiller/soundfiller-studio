          ...
          <div>
            ...
            <div className="flex flex-col gap-1">
              {tracks.map((track) => (
                <div
                  key={track.id}
                  className="flex items-center gap-3 px-3 py-2 text-xs font-mono cursor-pointer transition-colors"
                  style={{
                    backgroundColor: 'rgba(255,255,255,0.02)',
                    border: '1px solid rgba(255,255,255,0.06)',
                  }}
                  onClick={() => {
                    if (track.status === 'done' && track.result) {
                ...
                >
                  ...
                  <div className="w-2 h-2 rounded-full shrink-0" style={{
                    backgroundColor:
                      track.status === 'queued' ? 'rgba(255,255,255,0.3)' :
                  ...
                  <span className="flex-1 truncate text-white/80">{track.filename}</span>

                  {track.status === 'queued' && (
                    <span className="text-white/40 text-[10px]">Queued</span>
                  )}
                  {track.status === 'analysing' && (
                    <span className="text-[var(--color-studio-accent,#50FA7B)] text-[10px]">Analysing...</span>
                  )}
                  {track.status === 'done' && track.result && (
                    <span className="text-white/50 text-[10px] flex items-center gap-2">
                      <span>{Math.round(track.result.bpm)} BPM</span>
                      <span>{track.result.key_camelot}</span>
                      <span>{formatDuration(track.result.duration_seconds)}</span>
                      <span className="text-white/30">→ click to view</span>
                    </span>
                  )}
                  {track.status === 'error' && (
                  ...
                  <button
                    ...
                    className="text-white/20 hover:text-red-400 transition-colors no-radius text-[10px] ml-1"
                  >
                    ✕
                  </button>
                </div>
